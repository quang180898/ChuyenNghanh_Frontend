...
import { PopoverDropdown } from 'components/base/Popover'
...
const Sidebar = () => {
    ...
    useEffect(() => {
        dispatch(accountAction.getInfoProfile({ user_id: 1 }))
    },[])

    useEffect(() => {
        if(infoProfile) {
            if(infoProfile.success) {
                if(infoProfile.detail.permission_code === 1) {
                    setDataUser(infoProfile.detail)
                }
            }
        }
                ...
                <div className="avartar">
                    <div className="dropdown sub-dropdown">
                        <div className="profile-pic rounded-circle position-relative" type="button"
                           >
                            <img className="profile-pic rounded-circle position-relative" src={`data:image/jpeg;base64, ${dataUser && dataUser.image_bytes}`}/>
                        ...
                        <div className="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <div className="dropdown-item">
                                <i className="fa fa-circle " /> {dataUser && dataUser.permission_name}
                            </div>
                        </div>
                    </div>
                </div>
                <div className="profile">
                    <p className="name" style={{ fontFamily: '"Open Sans", sans-serif !important', fontWeight: 600 }}> {dataUser && dataUser.name} </p>
                    <p className="position"> {dataUser && dataUser.permission_name} </p>
